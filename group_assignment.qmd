...
```{r}
pca_model <- gravier_clean_aug_long |> 
  group_by(gene) |> 
...
Create the PCA model with the Many models approach
...
```{r }
# It is just to confirm that the model works as it supposed to be
...
```{r}
gravier_estim <- unnest(gravier_model, augmented_data)
```
